//! # VerdantGolem Plugin Utilities (`verdantgolem-plugin-utils`)
//!
//! A fast, secure, and developer-friendly utility crate for VerdantGolem server plugins, providing:
...
//! - **Zero-Argument Updates & Online Licensing**: Check licenses and updates against official VerdantGolem Marketplace endpoints without manual arguments.
...
//!
//! ```rust,ignore
//! use verdantgolem_plugin_api::{Plugin, Context, register_plugin};
//! use verdantgolem_plugin_utils::{init, check_license_online, check_for_updates};
//!
//! struct MyPlugin;
//!
...
//!         let metadata = verdantgolem_plugin_utils::init(context)
//!             .map_err(|e| format!("Plugin initialization failed: {e}"))?;
//!
//!         // 2. Check license online against marketplace
//!         let license_check = verdantgolem_plugin_utils::check_license_online(None)
...
//!         if let Ok(update) = verdantgolem_plugin_utils::check_for_updates() {
...
/// Initializes `verdantgolem-plugin-utils` using the plugin's runtime `Context`.
///
...
/// Initializes `verdantgolem-plugin-utils` with explicit metadata (useful for tests or custom initialization).
///
/// # Errors
///
...
pub fn evaluate_license(grace_period_days: u32) -> LicenseStatus {
    let Some(folder) = get_data_folder() else {
        return LicenseStatus::Invalid("verdantgolem_plugin_utils has not been initialized".to_string());
    };
