...
impl TextComponentBase {
    ...
    #[must_use]
    pub fn to_nbt_compound(&self) -> verdantgolem_nbt::NbtCompound {
    ...
    pub fn to_nbt_compound_for_version(
    ...
    ) -> verdantgolem_nbt::NbtCompound {
        let mut compound = verdantgolem_nbt::NbtCompound::new();
        match &*self.content {
        ...
        if let Some(ref click) = self.style.click_event {
            let mut click_tag = verdantgolem_nbt::NbtCompound::new();
            match click {
        ...
        if let Some(ref hover) = self.style.hover_event {
            let mut hover_tag = verdantgolem_nbt::NbtCompound::new();
            if *version >= JavaMinecraftVersion::V_1_21_5 {
                ...
                match hover {
                    ...
                    HoverEvent::ShowItem { id, count } => {
                        hover_tag.put_string("action", "show_item".to_string());
                        let mut contents = verdantgolem_nbt::NbtCompound::new();
    ...
    pub fn to_nbt_tag_for_version(
        &self,
        version: &JavaMinecraftVersion,
    ) -> verdantgolem_nbt::tag::NbtTag {
        if *version >= JavaMinecraftVersion::V_1_20_3
            && self.style.is_empty()
            && self.extra.is_empty()
            && let TextContent::Text { text } = &*self.content
        {
            verdantgolem_nbt::tag::NbtTag::String(text.to_string().into_boxed_str())
        } else {
            verdantgolem_nbt::tag::NbtTag::Compound(self.to_nbt_compound_for_version(version))
        }
    }
...
fn nbt_compound_to_json(compound: &verdantgolem_nbt::NbtCompound) -> serde_json::Value {
    let mut map = serde_json::Map::new();
...
fn nbt_tag_to_json(tag: &verdantgolem_nbt::tag::NbtTag) -> serde_json::Value {
    match tag {
        verdantgolem_nbt::tag::NbtTag::End => serde_json::Value::Null,
        verdantgolem_nbt::tag::NbtTag::Byte(b) => serde_json::json!(*b),
        verdantgolem_nbt::tag::NbtTag::Short(s) => serde_json::json!(*s),
        verdantgolem_nbt::tag::NbtTag::Int(i) => serde_json::json!(*i),
        verdantgolem_nbt::tag::NbtTag::Long(l) => serde_json::json!(*l),
        verdantgolem_nbt::tag::NbtTag::Float(f) => serde_json::json!(*f),
        verdantgolem_nbt::tag::NbtTag::Double(d) => serde_json::json!(*d),
        verdantgolem_nbt::tag::NbtTag::ByteArray(arr) => {
            serde_json::Value::Array(arr.iter().map(|&x| serde_json::json!(x)).collect())
        }
        verdantgolem_nbt::tag::NbtTag::String(s) => serde_json::Value::String(s.to_string()),
        verdantgolem_nbt::tag::NbtTag::List(list) => {
            serde_json::Value::Array(list.iter().map(nbt_tag_to_json).collect())
        }
        verdantgolem_nbt::tag::NbtTag::Compound(c) => nbt_compound_to_json(c),
        verdantgolem_nbt::tag::NbtTag::IntArray(arr) => {
            serde_json::Value::Array(arr.iter().map(|&x| serde_json::json!(x)).collect())
        }
        verdantgolem_nbt::tag::NbtTag::LongArray(arr) => {
            serde_json::Value::Array(arr.iter().map(|&x| serde_json::json!(x)).collect())
        }
    }
}
...
impl TextComponent {
    ...
    #[deprecated(
        since = "0.1.0",
        note = "Use the `verdantgolem_macros::translate_java!` macro instead for compile-time translation key and parameter checking."
    )]
    #[must_use]
    ...
    #[deprecated(
        since = "0.1.0",
        note = "Use the `verdantgolem_macros::translate_cross!` macro instead for compile-time translation key and parameter checking."
    )]
    #[must_use]
...
impl TextComponent {
    ...
    #[must_use]
    pub fn player_sprite(profile: verdantgolem_nbt::NbtCompound, hat: bool) -> Self {
        Self(TextComponentBase {
    ...
    pub fn encode_for_version(&self, version: &JavaMinecraftVersion) -> Box<[u8]> {
        ...
        let mut writer = verdantgolem_nbt::serializer::NbtWriteHelperJava::new(&mut bytes);
...
pub struct ProfileNbt(pub verdantgolem_nbt::NbtCompound);
...
mod test {
    ...
    fn serialize_text_component() {
        ...
        let decoded = verdantgolem_nbt::Nbt::read_unnamed(&mut reader).unwrap();
