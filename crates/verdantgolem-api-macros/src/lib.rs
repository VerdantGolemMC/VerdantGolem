...
pub fn plugin_impl(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        use verdantgolem::plugin::PluginFuture;
        ...
        #[unsafe(no_mangle)]
        pub static METADATA: std::sync::LazyLock<verdantgolem::plugin::PluginMetadata> = std::sync::LazyLock::new(|| {
            verdantgolem::plugin::PluginMetadata {
        ...
        #[unsafe(no_mangle)]
        pub static PUMPKIN_API_VERSION: u32 = verdantgolem::plugin::PLUGIN_API_VERSION;

        #input_struct

        impl verdantgolem::plugin::Plugin for #struct_ident {
            #(#methods)*
        }

        #[unsafe(no_mangle)]
        pub fn plugin() -> Box<dyn verdantgolem::plugin::Plugin> {
