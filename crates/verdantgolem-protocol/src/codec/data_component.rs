...
impl DataComponentCodec<Self> for CustomNameImpl {
    ...
    fn deserialize(seq: &mut impl NetworkReadExt) -> Result<Self, ReadingError> {
        ...
        Ok(Self {
            name: verdantgolem_util::text::TextComponent::text(String::from(name)),
        })
    }
}
...
impl DataComponentCodec<Self> for LoreImpl {
    fn serialize(&self, seq: &mut impl NetworkWriteExt) -> Result<(), WritingError> {
        ...
        for line in &self.lines {
            seq.write_slice(
                &line.encode_for_version(&verdantgolem_util::version::JavaMinecraftVersion::V_26_2),
            )?;
        }
        Ok(())
    }
...
impl DataComponentCodec<Self> for ItemNameImpl {
    fn serialize(&self, seq: &mut impl NetworkWriteExt) -> Result<(), WritingError> {
        let mut name = verdantgolem_nbt::compound::NbtCompound::new();
...
fn deserialize_item_stack_template(
    seq: &mut impl NetworkReadExt,
) -> Result<verdantgolem_data::item_stack::ItemStack, ReadingError> {
