...
impl<'a> DialogNBT<'a> {
    ...
    #[must_use]
    pub const fn from_nbt(compound: &'a verdantgolem_nbt::compound::NbtCompound) -> Self {
    ...
    pub fn write_packet_data(
        ...
        match &self.0 {
            ...
            DialogNBTSource::Nbt(nbt) => {
                let tag = verdantgolem_nbt::tag::NbtTag::Compound((*nbt).clone());
...
pub enum DialogNBTSource<'a> {
    Struct(&'a Dialog),
    Nbt(&'a verdantgolem_nbt::compound::NbtCompound),
}
