...
impl<'a, R: NetworkReadExt + ?Sized> verdantgolem_nbt::deserializer::NbtDataSource<'a>
    for NetworkReadDataSource<'a, R>
{
    fn read_u8(&mut self) -> Result<u8, verdantgolem_nbt::Error> {
        self.0.get_u8().map_err(|e| {
            verdantgolem_nbt::Error::Incomplete(std::io::Error::new(
    ...
    fn read_bytes(&mut self, buf: &mut [u8]) -> Result<(), verdantgolem_nbt::Error> {
    ...
    fn seek_relative(&mut self, offset: i64) -> Result<(), verdantgolem_nbt::Error> {
        if offset < 0 {
            return Err(verdantgolem_nbt::Error::Incomplete(std::io::Error::new(
    ...
    fn read_string(&mut self, len: usize) -> Result<Cow<'a, str>, verdantgolem_nbt::Error> {
        ...
        let string = cesu8::from_java_cesu8(&buf)
            .map_err(|_| verdantgolem_nbt::Error::Cesu8DecodingError)?;
        Ok(Cow::Owned(string.into_owned()))
    }

    fn read_byte_array(&mut self, len: usize) -> Result<Cow<'a, [i8]>, verdantgolem_nbt::Error> {
        let mut buf = vec![0u8; len];
        self.0.read_bytes_to_buf(&mut buf).map_err(|e| {
            verdantgolem_nbt::Error::Incomplete(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
...
pub trait NetworkReadExt {
    ...
    fn get_nbt_with_version(
        ...
        if *version >= JavaMinecraftVersion::V_1_8 {
            ...
            if tag_id == verdantgolem_nbt::END_ID {
                return Ok(None);
            }
            let mut helper =
                verdantgolem_nbt::deserializer::NbtReadHelperJava::new(NetworkReadDataSource(self));
            if *version < JavaMinecraftVersion::V_1_20_2 {
                let _name = helper
                    .get_string()
                    .map_err(|e| ReadingError::Message(e.to_string()))?;
            }
            let tag = if tag_id == verdantgolem_nbt::COMPOUND_ID {
                NbtTag::Compound(
            ...
            let mut helper = verdantgolem_nbt::deserializer::NbtReadHelperJava::new(&mut cursor);
            let tag_id = helper
                .get_u8()
                .map_err(|e| ReadingError::Message(e.to_string()))?;
            if tag_id == verdantgolem_nbt::END_ID {
...
impl<'a> NetworkReadSliceExt<'a> for &'a [u8] {
    ...
    fn get_nbt_borrowed(
        ...
        if *version >= JavaMinecraftVersion::V_1_8 {
            ...
            if (*self)[0] == verdantgolem_nbt::END_ID {
                *self = &(*self)[1..];
                return Ok(None);
            }
            let mut cursor = std::io::Cursor::new(*self);
    ...
    fn write_nbt_with_version(
        ...
        if *version >= JavaMinecraftVersion::V_1_8 {
                ...
                && !matches!(tag, NbtTag::End)
            {
                ...
                self.write_u8(verdantgolem_nbt::END_ID)?;
            }
        } else {
            // <= 1.7.10
            if let Some(tag) = data
