...
use verdantgolem_data::recipes::{CraftingRecipeTypes, RECIPES_CRAFTING};
...
use verdantgolem_protocol::codec::recipe::{DynamicRecipe, OwnedCraftingRecipe};
...
impl ResultSlot {
    ...
    fn refill_output(&self) -> ItemStack {
        let result = if let Some(matched) = self.match_recipe() {
            ...
            let item = verdantgolem_data::item::Item::from_registry_key(key)
