...
use verdantgolem_util::permission::PermissionManager;
...
pub struct Server {
    ...
    pub bedrock_oidc_keys: Arc<OnceCell<(String, verdantgolem_util::jwt::Jwks)>>,
    /// Cached Bedrock server private key (process-lifetime). Generated on first Bedrock login and reused.
    pub bedrock_private_key: OnceCell<Arc<verdantgolem_util::p384::ecdsa::SigningKey>>,
...
impl Server {
                ...
                .enabled
        {
            ...
            server.spawn_task(async move {
                let auth = server_clone
                    ...
                    .authentication
                    .clone();
                let keys = match verdantgolem_util::jwt::fetch_oidc_jwks(
                    auth.url.as_deref(),
    ...
    pub fn create_world(self: &Arc<Self>, name: String, dimension: Dimension) -> Arc<World> {
        ...
        let level =
            verdantgolem_world::dimension::into_level(dimension.clone(), &config, world_path, seed);
    ...
    pub fn reload_datapacks(&self, server: &Arc<Self>) {
        ...
        for player in self.get_all_players() {
            if let crate::net::ClientPlatform::Java(java_client) = player.client.as_ref() {
                let add_packet = verdantgolem_protocol::java::client::play::CRecipeBookAdd::new(
                    true,
                    &dynamic_recipes,
                );
    ...
    pub fn remove_player(&self, player: &Player) {
        player.increment_stat(
            ...
            verdantgolem_data::statistic::CustomStatistic::LeaveGame as i32,
            1,
        );
    ...
    pub fn update_tick_times(&self, tick_duration_nanos: i64) {
        ...
        let packet = verdantgolem_protocol::java::client::play::CDebugSample::new(
            &sample_slice,
            verdantgolem_protocol::codec::var_int::VarInt(0),
        );
        for world in self.worlds.load().iter() {
            let players = world.players.load();
            let recipients = players
                ...
                .filter(|player| {
                    player.subscribed_debug_sample.load(Ordering::Relaxed)
                        && player.permission_lvl.load() >= verdantgolem_util::PermissionLvl::Two
                })
