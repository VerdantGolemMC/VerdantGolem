...
use verdantgolem_data::chunk_gen_settings::GenerationSettings;
...
use verdantgolem_data::fluid::FluidState;
...
use verdantgolem_inventory::crafting::recipe_provider::RecipeProvider;
use verdantgolem_inventory::screen_handler::InventoryPlayer;
use verdantgolem_nbt::compound::NbtCompound;
use verdantgolem_protocol::bedrock::client::set_actor_data::{CSetActorData, PropertySyncData};
use verdantgolem_protocol::bedrock::client::start_game::{CStartGame, ServerTelemetryData};
...
use verdantgolem_protocol::java::client::play::{CSetEntityMetadata, Metadata};
...
use verdantgolem_world::inventory::Clearable;
use verdantgolem_world::world::{GetBlockError, WorldPortalExt};
...
fn bedrock_chest_block_actor(state_id: BlockStateId, position: BlockPos) -> Option<NbtCompound> {
    ...
    if !block.has_tag(&verdantgolem_data::tag::Block::C_CHESTS_WOODEN)
        && !block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_COPPER_CHESTS)
    {
        return None;
    }
...
impl World {
    ...
    pub fn send_remove_mob_effect(&self, entity: &Entity, effect_type: &'static StatusEffect) {
        ...
        let be_packet = verdantgolem_protocol::bedrock::client::CMobEffect {
            target_runtime_id: VarULong(entity.entity_id as u64),
            event_id: verdantgolem_protocol::bedrock::client::CMobEffect::EVENT_REMOVE,
    ...
    pub fn send_add_mob_effect(&self, entity: &Entity, effect: &verdantgolem_data::potion::Effect) {
        let chunk_pos = entity.chunk_pos.load();
        let mut flags: i8 = 0;
        if effect.ambient {
            flags |= 0x01;
        ...
        let be_packet = verdantgolem_protocol::bedrock::client::CMobEffect {
            target_runtime_id: VarULong(entity.entity_id as u64),
            event_id: verdantgolem_protocol::bedrock::client::CMobEffect::EVENT_ADD,
    ...
    fn broadcast_java_grouped<P: ClientPacket>(
        ...
        for (version, recipients) in recipients_by_version {
            let packet_data = match JavaClient::serialize_packet_for_version(packet, version) {
                ...
                Err(verdantgolem_protocol::ser::WritingError::UnsupportedVersion(_)) => {
                    continue;
                }
                Err(err) => {
                    error!(
    ...
    fn component_to_bedrock_text(message: &TextComponent) -> SText<'static> {
        match &*message.0.content {
            ...
            _ => SText::system_message(
                message
                    .0
                    .to_bedrock_legacy(verdantgolem_util::translation::Locale::EnUs),
            ),
        }
    }
                ...
                if tracked_count > 4096 {
                    recipient.kick(
                        ...
                        &TextComponent::translate_cross(
                            verdantgolem_data::translation::java::MULTIPLAYER_DISCONNECT_TOO_MANY_PENDING_CHATS,
                            verdantgolem_data::translation::java::MULTIPLAYER_DISCONNECT_TOO_MANY_PENDING_CHATS,
                            [],
                        ),
                    );
    ...
    pub fn play_sound_event(
        &self,
        sound: &verdantgolem_data::data_component_impl::IdOr<
            verdantgolem_data::data_component_impl::SoundEvent,
        >,
        ...
        for (chunk_section, updates) in block_state_updates_by_chunk_section {
            ...
            if updates.len() == 1 {
                ...
                self.broadcast_to_chunk_editioned(
                    ...
                    &verdantgolem_protocol::bedrock::client::CUpdateBlock::new(
                        block_pos,
                        be_block_id as u32,
                    ),
                );
                ...
                for (block_pos, block_state_id) in &updates {
                    let be_block_id = BlockState::to_be_network_id(*block_state_id);
                    let update_packet = verdantgolem_protocol::bedrock::client::CUpdateBlock::new(
                        *block_pos,
                        be_block_id as u32,
                    );
            ...
            for (block_pos, block_state_id) in &updates {
                ...
                let packet = verdantgolem_protocol::bedrock::client::CUpdateBlock::with_layer(
                    *block_pos,
    ...
    pub async fn spawn_bedrock_player(
        ...
        static BEDROCK_CRAFTING_DATA: std::sync::OnceLock<
            Vec<verdantgolem_protocol::bedrock::client::BedrockRecipe>,
        > = std::sync::OnceLock::new();
        ...
        let start_game = CStartGame {
            ...
            // Bedrock represents the local player at eye height; VerdantGolem stores feet position.
            ...
            level_settings,
            level_id: String::new(),
            level_name: "VerdantGolem world".to_string(),
            ...
            server_version: "VerdantGolem Rust Server".to_string(),
            compound_id: 10,
        ...
        let (groups, entries) = CREATIVE_CONTENT.get_or_init(|| {
            let groups = verdantgolem_data::bedrock_creative::CREATIVE_GROUPS
                .iter()
                .map(|g| {
                    ...
                    let icon_item = if g.icon_item_id != 0 {
                        NetworkItemDescriptor {
                            ...
                            nbt_data: verdantgolem_nbt::Nbt::default(),
            ...
            let entries = verdantgolem_data::bedrock_creative::CREATIVE_ENTRIES
                .iter()
                .enumerate()
        ...
        let bedrock_recipes = BEDROCK_CRAFTING_DATA.get_or_init(|| {
            use verdantgolem_data::item::{Item, JavaToBedrockItemMapping};
            use verdantgolem_data::recipes::{CraftingRecipeTypes, RecipeIngredientTypes};
            use verdantgolem_protocol::bedrock::client::{
                BedrockRecipe, BedrockShapedRecipe, BedrockShapelessRecipe, ItemDescriptorCount,
                RecipeUnlockRequirement,
            };
            use verdantgolem_protocol::bedrock::network_item::NetworkItemDescriptor;
            use verdantgolem_protocol::codec::{var_int::VarInt, var_uint::VarUInt};

            let mut mapped_recipes = Vec::new();
            let mut network_id_counter = 1u32;

            for recipe in verdantgolem_data::recipes::RECIPES_CRAFTING {
                let map_ingredient = |ing: &RecipeIngredientTypes| -> ItemDescriptorCount {
                    let item_key = match ing {
                        ...
                        RecipeIngredientTypes::Tagged(tag) => {
                            ...
                            verdantgolem_data::tag::get_tag_ids(
        ...
        let crafting_data = verdantgolem_protocol::bedrock::client::CCraftingData {
            ...
            .filter(|p| p.gameprofile.id != gameprofile.id)
        {
            ...
            let ex_be_mob_equipment = verdantgolem_protocol::bedrock::client::CMobEquipment {
    ...
    pub async fn spawn_java_player(
            ...
            && client.version.load() >= JavaMinecraftVersion::V_1_13
        {
            ...
            for &key in verdantgolem_data::tag::RegistryKey::NETWORK_KEYS {
                if verdantgolem_data::tag::get_registry_key_tags(version, key)
                    .is_some_and(|map| !map.is_empty())
                {
                    tags.push(key);
                }
            }
            let packet = verdantgolem_protocol::java::client::play::CUpdateTagsPlay::new(&tags);
            if let Ok(packet_data) = JavaClient::serialize_packet_for_version(&packet, version) {
            ...
            .filter(|c| c.gameprofile.id != id)
        {
...

            {
                ...
                let be_mob_equipment = verdantgolem_protocol::bedrock::client::CMobEquipment {
                    target_runtime_id: (existing_player.entity_id() as u64).into(),
                    item: (&held_item).into(),
                    slot: 0,
                    selected_slot: 0,
        ...
        player
            .send_packet_now_editioned(
                ...
                &verdantgolem_protocol::bedrock::client::CSetSpawnPosition {
                    spawn_position_type:
                        verdantgolem_protocol::bedrock::client::SpawnPositionType::WorldRespawn,
    ...
    pub fn get_fluid(&self, position: &BlockPos) -> &'static verdantgolem_data::fluid::Fluid {
    ...
    pub fn get_block_and_fluid(
        ...
        &'static verdantgolem_data::Block,
        &'static verdantgolem_data::fluid::Fluid,
    ) {
        let id = self.get_block_state_id(position);
        let block = Block::from_state_id(id);
    ...
    pub fn add_block_entity(&self, block_entity: Arc<dyn BlockEntity>) {
        ...
        if let Some(nbt) = &block_entity_nbt {
            let bytes = verdantgolem_nbt::Nbt::from(nbt.clone()).write_unnamed();
            self.broadcast_to_chunk(
                chunk_pos,
                &CBlockEntityData::new(
...

        {
            ...
            if !custom_data.is_empty() {
                let custom_data_path = self
                    ...
                    .root_folder
                    .join("pumpkin_custom_data.nbt");
                let nbt = verdantgolem_nbt::Nbt::from(custom_data.clone());
    ...
    pub fn set_custom_data(
        ...
        let mut namespace_data = custom_data
            ...
            .and_then(|tag| match tag {
                verdantgolem_nbt::tag::NbtTag::Compound(compound) => Some(compound),
                _ => None,
            })
            .unwrap_or_default();
        ...
        custom_data.child_tags.insert(
            namespace.into(),
            verdantgolem_nbt::tag::NbtTag::Compound(namespace_data),
        );
    }
    ...
    pub fn remove_custom_data(&self, namespace: &str, key: &str) {
        ...
        let Some(verdantgolem_nbt::tag::NbtTag::Compound(mut namespace_data)) =
