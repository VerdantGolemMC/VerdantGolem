...
fn register_level_0_permissions(registry: &PermissionRegistry) {
    ...
    registry
        .register_permission(Permission::new(
            "pumpkin:command.verdantgolem",
            "Shows information about the VerdantGolem server",
            PermissionDefault::Allow,
        ))
        .unwrap_or_else(|e| tracing::warn!("{e}"));
    registry
...
#[cfg(test)]
mod override_tests {
    use verdantgolem_config::{CommandOverride, CommandsConfig};
    use verdantgolem_util::PermissionLvl;
    use verdantgolem_util::permission::{PermissionDefault, PermissionManager};
