...
fn from_wasm_sign_text(text: SignText) -> InternalText {
    ...
    InternalText::from(verdantgolem_nbt::tag::NbtTag::Compound({
        ...
        nbt.put_list(
            ...
            messages
                .iter()
                .map(|s| verdantgolem_nbt::tag::NbtTag::String(s.clone().into()))
                .collect(),
        );
        nbt
    }))
...
impl HostContainerBlockEntity for PluginHostState {
    ...
    async fn set_stack(
        ...
        let stack = match stack_res {
            ...
            None => verdantgolem_data::item_stack::ItemStack::EMPTY.clone(),
        };
