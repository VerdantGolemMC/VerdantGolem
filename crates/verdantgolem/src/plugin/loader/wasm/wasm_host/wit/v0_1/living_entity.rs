...
) -> verdantgolem_data::data_component_impl::EquipmentSlot {
...
#[must_use]
pub fn from_wit_damage_type(wit: WitDamageType) -> verdantgolem_data::damage::DamageType {
    verdantgolem_data::damage::DamageType::from_id(wit as u8)
        .unwrap_or(verdantgolem_data::damage::DamageType::GENERIC)
}
    ...
    async fn clear_equipment(&mut self, this: Resource<WitLivingEntity>) -> wasmtime::Result<()> {
        ...
        if let Some(living) = entity.get_living_entity() {
            ...
            let slots_to_clear: Vec<(
                verdantgolem_data::data_component_impl::EquipmentSlot,
                verdantgolem_data::item_stack::ItemStack,
            )> = equipment
                .equipment
                .drain()
