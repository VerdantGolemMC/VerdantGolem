...
impl PluginHostState {
    ...
    ) -> wasmtime::Result<Arc<Mutex<verdantgolem_data::item_stack::ItemStack>>> {
        self.resource_table
...
impl HostItemStack for PluginHostState {
    async fn new(
        ...
        let item = verdantgolem_data::item::Item::from_registry_key(
            registry_key
                .strip_prefix("minecraft:")
                .unwrap_or(&registry_key),
        )
        .unwrap_or(&verdantgolem_data::item::Item::AIR);
        let stack = verdantgolem_data::item_stack::ItemStack::new(count, item);
