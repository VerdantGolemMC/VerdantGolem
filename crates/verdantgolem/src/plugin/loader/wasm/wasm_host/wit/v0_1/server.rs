...
impl pumpkin::plugin::server::HostServer for PluginHostState {
    ...
    async fn get_difficulty(&mut self, res: Resource<Server>) -> wasmtime::Result<Difficulty> {
        ...
        Ok(match resource.provider.get_difficulty() {
            verdantgolem_util::Difficulty::Peaceful => Difficulty::Peaceful,
            verdantgolem_util::Difficulty::Easy => Difficulty::Easy,
            verdantgolem_util::Difficulty::Normal => Difficulty::Normal,
            verdantgolem_util::Difficulty::Hard => Difficulty::Hard,
        })
    }
    ...
    async fn create_world(
        ...
        let internal_dim = match dimension {
            Dimension::Overworld => verdantgolem_data::dimension::Dimension::OVERWORLD,
            Dimension::Nether => verdantgolem_data::dimension::Dimension::THE_NETHER,
            Dimension::End => verdantgolem_data::dimension::Dimension::THE_END,
        };
    ...
    async fn get_all_advancement_ids(
        ...
        let ids = verdantgolem_data::Advancement::get_identifier_list()
            .iter()
    ...
    async fn get_all_enchantment_ids(
        ...
        for enc in verdantgolem_data::enchantment::Enchantment::ALL {
        ...
        if removed {
            if let Some(player) = server.get_player_by_uuid(uuid) {
                ...
                player.set_permission_lvl(
                    server,
                    verdantgolem_util::PermissionLvl::Zero,
                    &command_dispatcher,
                );
            }
    ...
    async fn ban_player(
        ...
        let reason_text = options
            ...
            .map_or_else(
                ...
                verdantgolem_util::text::TextComponent::to_pretty_console,
            );
            ...
            && let Some(player) = server.get_player_by_uuid(uuid)
        {
            player.kick(
                ...
                &verdantgolem_util::text::TextComponent::text(reason_text.clone()),
            );
        }
            ...
            for player in to_kick {
                player.kick(
                    ...
                    &verdantgolem_macros::translate_cross!(
                        verdantgolem_data::translation::java::MULTIPLAYER_DISCONNECT_NOT_WHITELISTED,
                        verdantgolem_data::translation::bedrock::DISCONNECT_KICKED
                    ),
                );
            }
        }
