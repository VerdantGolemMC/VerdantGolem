    ...
    fn from_wasm_event(event: Event, state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockDamageEvent(data) => Self {
                player: consume_player(state, &data.player),
                block: &verdantgolem_data::Block::AIR,
                block_pos: from_wasm_block_position(data.block_pos),
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockIgniteEvent(data) => Self {
                block_pos: from_wasm_block_position(data.block_pos),
                igniting_block: &verdantgolem_data::Block::FIRE,
                player: None,
                cancelled: data.cancelled,
            },
            _ => panic!("unexpected event type"),
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockFromToEvent(data) => Self {
                ...
                block: &verdantgolem_data::Block::WATER,
                cancelled: data.cancelled,
            },
            _ => panic!("unexpected event type"),
        }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockFormEvent(data) => Self {
                block_pos: from_wasm_block_position(data.block_pos),
                block: &verdantgolem_data::Block::SNOW,
                cancelled: data.cancelled,
            },
            _ => panic!("unexpected event type"),
        }
    ...
    fn from_wasm_event(event: Event, _state: &mut PluginHostState) -> Self {
        match event {
            Event::BlockFadeEvent(data) => Self {
                block_pos: from_wasm_block_position(data.block_pos),
                block: &verdantgolem_data::Block::ICE,
                cancelled: data.cancelled,
            },
            _ => panic!("unexpected event type"),
        }
