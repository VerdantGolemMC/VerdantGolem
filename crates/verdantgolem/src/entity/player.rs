...
use verdantgolem_protocol::RawPacket;
use verdantgolem_protocol::bedrock::client::play_status::CPlayStatus;
use verdantgolem_protocol::bedrock::client::set_time::CSetTime;
use verdantgolem_protocol::bedrock::client::update_abilities::{Ability, CUpdateAbilities};
...
use verdantgolem_protocol::bedrock::server::{
...
impl JavaPlayer<'_> {
    pub async fn send_packet<C: verdantgolem_protocol::ClientPacket + Sync>(&self, packet: &C) {
...
impl BedrockPlayer<'_> {
    pub async fn send_packet<P: verdantgolem_protocol::BClientPacket + Sync>(&self, packet: &P) {
...
use verdantgolem_data::block_properties::{BlockProperties, HorizontalFacing};
use verdantgolem_data::damage::DamageType;
use verdantgolem_data::data_component_impl::{AttributeModifiersImpl, EnchantmentsImpl, Operation};
use verdantgolem_data::data_component_impl::{EquipmentSlot, EquippableImpl, ToolImpl, WeaponImpl};
use verdantgolem_data::effect::StatusEffect;
use verdantgolem_data::entity::{EntityPose, EntityStatus, EntityType};
...
use verdantgolem_data::statistic::StatisticCategory;
use verdantgolem_data::tag::Taggable;
use verdantgolem_data::{Block, BlockState, Enchantment, screen::WindowType, tag, translation};
use verdantgolem_inventory::player::{
...
use verdantgolem_inventory::sync_handler::SyncHandler;
use verdantgolem_macros::send_cancellable;
use verdantgolem_nbt::compound::NbtCompound;
use verdantgolem_nbt::tag::NbtTag;
use verdantgolem_protocol::IdOr;
use verdantgolem_protocol::SoundEvent;
use verdantgolem_protocol::bedrock::client::container_open::CContainerOpen;
use verdantgolem_protocol::bedrock::server::actor_event::{ActorEventID, SActorEvent};
use verdantgolem_protocol::codec::var_int::VarInt;
use verdantgolem_protocol::codec::var_long::VarLong;
use verdantgolem_protocol::codec::var_ulong::VarULong;
...
use verdantgolem_util::{GameMode, Hand};
use verdantgolem_world::biome;
use verdantgolem_world::cylindrical_chunk_iterator::Cylindrical;
use verdantgolem_world::level::{Level, SyncChunk, SyncEntityChunk};
...
use verdantgolem_world::chunk_system::ChunkLoading;
...
pub struct Player {
    ...
    pub compass_target: AtomicCell<Option<verdantgolem_util::math::position::BlockPos>>,
    pub respawn_location: AtomicCell<Option<verdantgolem_util::math::position::BlockPos>>,
    ...
    pub bedrock_skin: arc_swap::ArcSwap<verdantgolem_protocol::bedrock::client::Skin>,
...
impl Player {
    ...
    pub fn fetch_skin(properties: &[Property]) -> Option<verdantgolem_protocol::bedrock::client::Skin> {
        ...
        let mut skin = verdantgolem_protocol::bedrock::client::Skin::steve();
    ...
    pub fn new(
        ...
        let mut bedrock_skin = Self::fetch_skin(&properties)
            .unwrap_or_else(verdantgolem_protocol::bedrock::client::Skin::steve);
    ...
    pub fn set_display_name(&self, display_name: Option<TextComponent>) {
        ...
        *guard = display_name;
        ...
        world.broadcast_packet_all(&CPlayerInfoUpdate::new(
            ...
            &[verdantgolem_protocol::java::client::play::Player {
    ...
    pub fn open_ender_chest(self: &Arc<Self>) -> Option<u8> {
        self.increment_stat(
            ...
            verdantgolem_data::statistic::CustomStatistic::OpenEnderchest as i32,
            1,
        );
    ...
    pub fn attack(&self, victim: &Arc<dyn EntityBase>) {
        ...
        if let Some(strength) = self
            .living_entity
            .get_effect(&verdantgolem_data::effect::StatusEffect::STRENGTH)
        {
            damage += 3.0 * (f64::from(strength.amplifier) + 1.0);
        }
        if let Some(weakness) = self
            .living_entity
            .get_effect(&verdantgolem_data::effect::StatusEffect::WEAKNESS)
        {
    ...
    pub fn try_send_slot_set_packet(&self, packet: &CSetPlayerInventory) {
        match self.client.as_ref() {
            ...
            ClientPlatform::Bedrock(bedrock) => {
                ...
                use verdantgolem_protocol::codec::var_uint::VarUInt;
    ...
    pub fn damage_item_in_slot(&self, slot: &EquipmentSlot, amount: i32) -> bool {
        ...
        let updated = (result != verdantgolem_data::item_stack::DamageResult::Untouched)
        ...
        if let Some((result, updated_stack)) = updated {
            ...
            if result == verdantgolem_data::item_stack::DamageResult::Broken {
                if let Some(server) = self.world().server.upgrade()
                    && let Some(player_arc) = self.world().get_player_by_uuid(self.gameprofile.id)
                {
    ...
    pub fn set_respawn_point(
        ...
        self.client.try_enqueue_packet_editioned(
            ...
            &verdantgolem_protocol::bedrock::client::CSetSpawnPosition {
                spawn_position_type:
                    verdantgolem_protocol::bedrock::client::SpawnPositionType::PlayerRespawn,
    ...
    pub async fn calculate_respawn_point(&self) -> Option<CalculatedRespawnPoint> {
        type BedProperties = verdantgolem_data::block_properties::WhiteBedLikeProperties;
        type AnchorProperties = verdantgolem_data::block_properties::RespawnAnchorLikeProperties;
    ...
    pub fn sleep(&self, bed_head_pos: BlockPos) {
        ...
        self.get_entity().send_meta_data(
            &[Metadata::new(
                verdantgolem_data::tracked_data::player::SLEEPING_POS_ID,
                Some(bed_head_pos),
            )],
            None,
        );
    ...
    pub fn show_title(&self, text: &TextComponent, mode: &TitleMode) {
        match mode {
            TitleMode::Title => {
                self.client.try_enqueue_packet_editioned(
                    &CTitleText::new(text),
                    &verdantgolem_protocol::bedrock::client::set_title::CSetTitle::new(
                        verdantgolem_protocol::bedrock::client::TitleType::Title,
                        text.clone().get_text(),
                        0,
                        0,
                        0,
            ...
            TitleMode::SubTitle => {
                self.client.try_enqueue_packet_editioned(
                    &CSubtitle::new(text),
                    &verdantgolem_protocol::bedrock::client::set_title::CSetTitle::new(
                        verdantgolem_protocol::bedrock::client::TitleType::Subtitle,
                        text.clone().get_text(),
                        0,
                        0,
                        0,
    ...
    pub fn send_title_animation(&self, fade_in: i32, stay: i32, fade_out: i32) {
        self.try_enqueue_packet_editioned(
            ...
            &verdantgolem_protocol::bedrock::client::set_title::CSetTitle::new(
                verdantgolem_protocol::bedrock::client::TitleType::Times,
                String::new(),
                fade_in,
                stay,
                fade_out,
    ...
    pub fn get_movement_statistic(&self) -> statistics::CustomStatistic {
        ...
        if block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_CLIMBABLE) {
    ...
    pub fn send_difficulty_update(&self) {
        ...
        self.client.try_enqueue_packet_editioned(
            ...
            &verdantgolem_protocol::bedrock::client::CSetDifficulty {
    ...
    pub async fn send_editioned<
        J: verdantgolem_protocol::ClientPacket + Sync,
        B: verdantgolem_protocol::BClientPacket + Sync,
    >(
        &self,
        je_packet: &J,
        be_packet: &B,
    ...
    pub fn set_compass_target(&self, pos: verdantgolem_util::math::position::BlockPos) {
        use verdantgolem_protocol::java::client::play::CPlayerSpawnPosition;
        self.compass_target.store(Some(pos));
        self.try_send_client_packet(&CPlayerSpawnPosition::new(pos, 0.0, 0.0, String::new()));
    }

    pub fn get_compass_target(&self) -> Option<verdantgolem_util::math::position::BlockPos> {
        self.compass_target.load()
    }

    pub fn set_respawn_location(&self, pos: verdantgolem_util::math::position::BlockPos) {
        self.respawn_location.store(Some(pos));
    }

    pub fn get_respawn_location(&self) -> Option<verdantgolem_util::math::position::BlockPos> {
    ...
    pub fn get_target_block(
        ...
        world: &Arc<World>,
        max_distance: f64,
    ) -> Option<verdantgolem_util::math::position::BlockPos> {
        let (yaw, pitch) = (
            self.living_entity.entity.yaw.load(),
            self.living_entity.entity.pitch.load(),
        );
        ...
        let dir = verdantgolem_util::math::vector3::Vector3::new(
        ...
        send_cancellable! {{
            ...
            'after: {
               ...
               new_world.players.rcu(|current_list| {
                ...
                if new_world.dimension == verdantgolem_data::dimension::Dimension::THE_NETHER {
                ...
                } else if new_world.dimension == verdantgolem_data::dimension::Dimension::THE_END {
                    self.trigger_advancement(crate::entity::player::advancement::trigger::AdvancementTrigger::EnterDimension {
                ...
                match self.client.as_ref() {
                    ...
                    ClientPlatform::Bedrock(bedrock) => {
                        ...
                        let change_dim_packet = verdantgolem_protocol::bedrock::client::CChangeDimension {
    ...
    pub fn damage_with_context(
        ...
        if self
            ...
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .invulnerable
            && damage_type != verdantgolem_data::damage::DamageType::GENERIC_KILL
            && damage_type != verdantgolem_data::damage::DamageType::OUT_OF_WORLD
        {
            return false;
        }
        self.living_entity
    ...
    pub fn set_gamemode(self: &Arc<Self>, gamemode: GameMode) -> bool {
        ...
        self.client.try_enqueue_packet_editioned(
            ...
            &verdantgolem_protocol::bedrock::client::set_player_gamemode::CSetPlayerGameType {
    ...
    pub fn send_client_information(&self) {
        ...
        self.living_entity.entity.send_meta_data(
            &[
                // v26.x
                Metadata::new(
                    verdantgolem_data::tracked_data::player::PLAYER_MODE_CUSTOMISATION,
                    config.skin_parts,
                ),
                Metadata::new(
                    verdantgolem_data::tracked_data::player::PLAYER_MAIN_HAND,
                    config.main_hand as u8,
                ),
                // v1.21.x
                Metadata::new(
                    verdantgolem_data::tracked_data::player::PLAYER_MODE_CUSTOMIZATION_ID,
                    config.skin_parts,
                ),
                Metadata::new(
                    verdantgolem_data::tracked_data::player::MAIN_ARM_ID,
                    config.main_hand as u8,
                ),
            ],
            None,
    ...
    pub fn send_system_message_raw(&self, text: &TextComponent, overlay: bool) {
        ...
        let be_packet = match &*text.0.content {
            verdantgolem_util::text::TextContent::Translate {
                ...
                let parameters = with
                    .iter()
                    .map(verdantgolem_util::text::TextComponentBase::to_bedrock_string)
                    .collect();
                    ...
                    && let Ok(mut map_data) = map_data_arc.try_lock()
                {
                    ...
                    if map_data.dirty || tick_count % 10 == 0 {
                        ...
                        let decoration_type = if is_off_map {
                            &verdantgolem_data::map_decoration::MapDecorationType::PLAYER_OFF_MAP
                        } else {
                            &verdantgolem_data::map_decoration::MapDecorationType::PLAYER
                        };
    ...
    pub fn remove_effect(&self, effect_type: &'static StatusEffect) -> bool {
        ...
        self.try_send_client_packet(
            &verdantgolem_protocol::java::client::play::CRemoveMobEffect::new(
                self.entity_id().into(),
                effect_id,
            ),
        );
    ...
    pub fn close_handled_screen(&self) {
        ...
        self.try_enqueue_packet_editioned(
            ...
            &verdantgolem_protocol::bedrock::server::container_close::SContainerClose {
            ...
            && !stack.is_empty()
        {
            let mut extract_event =
                crate::plugin::api::events::inventory::furnace_extract::FurnaceExtractEvent::new(
                    self.clone(),
                    verdantgolem_util::math::position::BlockPos::new(0, 0, 0),
                    stack.item.registry_key.to_string(),
    ...
    pub fn swing_hand(&self, hand: Hand, all: bool) {
        ...
        let je_packet = verdantgolem_protocol::java::client::play::CEntityAnimation::new(
            VarInt(entity_id),
            animation,
        );
    ...
    pub fn has_advancement(
        &self,
        advancement: &'static verdantgolem_data::advancement::Advancement,
    ) -> bool {
    ...
    pub fn has_item_in_inventory(&self, item: &verdantgolem_data::item::Item) -> bool {
        let main_inv = self
            .inventory
            .main_inventory
            .read()
...
impl InventoryPlayer for Player {
    ...
    fn enqueue_set_held_item_packet(&self, packet: &CSetSelectedSlot) {
        self.try_enqueue_packet_editioned(
            packet,
            &verdantgolem_protocol::bedrock::client::CPlayerHotbar {
                selected_slot: verdantgolem_protocol::codec::var_uint::VarUInt(packet.slot as u32),
                container_id: 0,
    ...
    fn fire_enchant_item_event(
        ...
        option: i32,
        exp_level_cost: i32,
        enchantments_to_add: &mut Vec<(&'static verdantgolem_data::Enchantment, i32)>,
    ) -> bool {
        let Some(player_arc) = self.world().get_player_by_uuid(self.gameprofile.id) else {
            return false;
        };
