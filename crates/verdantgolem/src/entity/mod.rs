...
use verdantgolem_protocol::bedrock::client::{CAddActor, CSetActorMotion};
use verdantgolem_protocol::codec::var_long::VarLong;
use verdantgolem_protocol::java::client::play::{CUpdateEntityPos, CUpdateEntityPosRot};
...
use verdantgolem_util::math::vector3::Axis;
...
pub trait EntityBase: Send + Sync + std::any::Any {
    ...
    fn get_job_site_pos(&self) -> Option<verdantgolem_util::math::position::BlockPos> {
        None
    }

    fn get_home_pos(&self) -> Option<verdantgolem_util::math::position::BlockPos> {
        None
    }
...
impl Entity {
    ...
    pub fn try_use_portal(&self, portal_world: Arc<World>, pos: BlockPos) {
        ...
        if manager.is_none() {
            ...
            let (block, state) = world.get_block_and_state(&pos);
            let source_axis = (block == &verdantgolem_data::Block::NETHER_PORTAL).then(|| {
                let props = <verdantgolem_data::block_properties::NetherPortalLikeProperties as verdantgolem_data::block_properties::BlockProperties>::from_state_id(state.id, block);
                props.axis
            });
            ...
            *manager = Some(new_manager);
            ...
            if manager.source_portal.is_none() {
                let (block, state) = world.get_block_and_state(&pos);
                if block == &verdantgolem_data::Block::NETHER_PORTAL {
                    let props = <verdantgolem_data::block_properties::NetherPortalLikeProperties as verdantgolem_data::block_properties::BlockProperties>::from_state_id(state.id, block);
...
impl Entity {
    pub fn write_nbt(&self, nbt: &mut NbtCompound) {
        ...
        if let Some(custom_name) = &**self.custom_name.load()
            && let Ok(name_json) = verdantgolem_util::serde_json::to_string(custom_name)
        {
    ...
    pub fn read_nbt_non_mut(&self, nbt: &NbtCompound) {
        ...
        if let Some(name_json) = nbt.get_string("CustomName")
            && let Ok(component) = verdantgolem_util::serde_json::from_str(name_json)
        {
