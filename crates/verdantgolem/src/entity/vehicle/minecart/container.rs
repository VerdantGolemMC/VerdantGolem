...
use verdantgolem_inventory::generic_container_screen_handler::{create_generic_9x3, create_hopper};
...
use verdantgolem_data::chest_loot_table::get_chest_loot_table;
...
impl MinecartInventory {
    ...
    pub(super) fn write_nbt(&self, nbt: &mut NbtCompound) {
        ...
        if let Some((loot_table, seed)) = loot_table {
            ...
            let mut list: Vec<verdantgolem_nbt::tag::NbtTag> = Vec::new();
            for (slot, stack) in items.iter().enumerate() {
                if !stack.is_empty() {
                    let mut compound = NbtCompound::new();
                    compound.put_byte("Slot", slot as i8);
                    stack.write_item_stack(&mut compound);
                    list.push(verdantgolem_nbt::tag::NbtTag::Compound(compound));
                }
            }
            nbt.put("Items", verdantgolem_nbt::tag::NbtTag::List(list));
        }
    }
