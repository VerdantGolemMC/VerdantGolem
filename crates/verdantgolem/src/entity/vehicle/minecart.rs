...
use verdantgolem_data::block_properties::{BlockProperties, PoweredRailLikeProperties};
...
const fn get_exits(
    shape: verdantgolem_data::block_properties::RailShape,
) -> (Vector3<f64>, Vector3<f64>) {
    use verdantgolem_data::block_properties::RailShape;
    match shape {
...
impl EntityBase for MinecartEntity {
    ...
    fn tick(&self, caller: &dyn EntityBase, _server: &Server) {
        ...
        if is_on_rails {
            use verdantgolem_data::block_properties::RailLikeProperties;
            use verdantgolem_data::block_properties::{RailShape, RailShapeStraight};
