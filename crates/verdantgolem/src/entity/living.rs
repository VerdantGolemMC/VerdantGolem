...
use verdantgolem_inventory::build_equipment_slots;
...
use verdantgolem_protocol::bedrock::client::take_item_actor::CTakeItemActor;
...
use verdantgolem_data::data_component_impl::food::{ConsumableImpl, ConsumeEffect};
...
use verdantgolem_data::item_stack::{DamageResult, ItemStack};
use verdantgolem_data::sound::SoundCategory;
use verdantgolem_data::{Block, Enchantment, translation};
use verdantgolem_data::{damage::DamageType, sound::Sound};
use verdantgolem_inventory::entity_equipment::EntityEquipment;
...
impl MetadataSerializer for EffectParticles {
    fn write_metadata(
        ...
        _version: &verdantgolem_util::version::JavaMinecraftVersion,
    ) -> Result<(), WritingError> {
...
impl LivingEntity {
    ...
    pub fn send_equipment_changes(&self, equipment: &[(EquipmentSlot, ItemStack)]) {
        ...
        for (slot, stack) in equipment {
            if *slot == EquipmentSlot::MAIN_HAND || *slot == EquipmentSlot::OFF_HAND {
                ...
                let be_packet = verdantgolem_protocol::bedrock::client::CMobEquipment {
                    target_runtime_id: (self.entity_id() as u64).into(),
                    item: verdantgolem_protocol::bedrock::network_item::NetworkItemStackDescriptor::from(
                        stack,
                    ),
                    slot: 0,
                    selected_slot: 0,
    ...
    fn set_living_flag(&self, flag: u8, value: bool) {
        ...
        let bedrock_meta = (flag == Self::USING_ITEM_FLAG).then(|| {
            let index =
                verdantgolem_protocol::bedrock::client::set_actor_data::entity_data_flag::USING_ITEM;
            let mask = 1i64 << index;
            if value {
            ...
            meta.set(
                verdantgolem_protocol::bedrock::client::set_actor_data::entity_data_key::FLAGS,
                verdantgolem_protocol::bedrock::client::set_actor_data::MetadataValue::Int64(
    ...
    pub fn add_effect(&self, effect: Effect) {
        ...
        if effect.effect_type == &StatusEffect::INSTANT_HEALTH {
            ...
            if !effect.effect_type.attribute_modifiers.is_empty() {
                ...
                let mut touched_attrs: Vec<verdantgolem_data::attributes::Attributes> = Vec::new();
                for m in effect.effect_type.attribute_modifiers {
    ...
    pub fn swing_hand(&self) {
        ...
        let je_packet = verdantgolem_protocol::java::client::play::CEntityAnimation::new(
            entity_id.into(),
            verdantgolem_protocol::java::client::play::Animation::SwingMainArm,
        );
            ...
            let tool = if let Some(cause_ent) = cause {
                    ...
                    .downcast_ref::<crate::entity::player::Player>()
                {
                    let hand_stack = player
                        .inventory()
                        .get_stack_in_hand(verdantgolem_util::Hand::Right);
    ...
    fn damage_armor_items(&self, caller: &dyn EntityBase, damage_amount: f32) {
        ...
        for (slot_index, mut stack, slot) in armor_slots {
            ...
            if takes_damage {
                ...
                if slot_result != verdantgolem_data::item_stack::DamageResult::Untouched {
                    if slot_result == verdantgolem_data::item_stack::DamageResult::Broken {
                        let world = self.entity.world.load();
...
impl LivingEntity {
    pub fn write_living_nbt(&self, nbt: &mut NbtCompound) {
        ...
        {
            let effects_vec: Vec<verdantgolem_data::potion::Effect> = {
                let effects = self
                    .active_effects
                    .lock()
                    .unwrap_or_else(std::sync::PoisonError::into_inner);
            ...
            if !effects_vec.is_empty() {
                ...
                for effect in effects_vec {
                    let mut effect_nbt = verdantgolem_nbt::compound::NbtCompound::new();
    ...
    pub fn damage_with_context(
        ...
        if clamped_health <= 0.0 {
            ...
            world.send_entity_status(
                &self.entity,
                verdantgolem_data::entity::EntityStatus::Death,
                None,
            );
        }
        ...
        for consume_effect in consumable.effects.iter() {
            match consume_effect {
                ...
                ConsumeEffect::RemoveEffects(idset) => {
                    if let verdantgolem_data::data_component_impl::IDSet::IDs(ids) = idset {
...
mod tests {
    ...
    fn regeneration_particle_metadata_uses_vanilla_argb_color() {
        ...
        metadata
            .write(
                &mut bytes,
                &verdantgolem_util::version::JavaMinecraftVersion::V_26_2,
            )
            .unwrap();
