...
use verdantgolem_data::statistic::{CustomStatistic, StatisticCategory};
use verdantgolem_data::villager::{
...
use verdantgolem_protocol::java::client::play::CMerchantOffers;
...
fn add_offers_from_trade_set(
    offers: &mut Vec<verdantgolem_protocol::java::client::play::MerchantOffer>,
...
impl Mob for WanderingTraderEntity {
    ...
    fn mob_write_nbt(&self, nbt: &mut NbtCompound) {
        ...
        if let Some(target) = wander_target {
            nbt.put(
                "wander_target",
                verdantgolem_nbt::tag::NbtTag::IntArray(vec![target.0.x, target.0.y, target.0.z]),
            );
        }
        ...
        if !offers.is_empty() {
            ...
            for offer in offers.iter() {
                ...
                recipes.push(verdantgolem_nbt::tag::NbtTag::Compound(recipe));
            }
            let mut offers_compound = NbtCompound::new();
            offers_compound.put("Recipes", verdantgolem_nbt::tag::NbtTag::List(recipes));
