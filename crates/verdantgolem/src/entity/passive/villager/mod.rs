...
use verdantgolem_data::tag::{Enchantment as EnchantmentTag, Taggable};
...
use verdantgolem_protocol::java::client::play::{CMerchantOffers, Metadata};
...
pub(crate) fn apply_potion(stack: &mut ItemStack, potion_name: &str) {
    use verdantgolem_data::data_component::DataComponent;
    use verdantgolem_data::data_component_impl::{DataComponentImpl, PotionContentsImpl};

    let Some(potion) = verdantgolem_data::potion::Potion::from_name(
        potion_name
...
pub struct VillagerEntity {
    ...
    pub offers: std::sync::Mutex<Vec<verdantgolem_protocol::java::client::play::MerchantOffer>>,
...
impl VillagerEntity {
    ...
    fn create_explorer_map(&self, destination: &str) -> Option<ItemStack> {
        use verdantgolem_data::data_component::DataComponent;
        use verdantgolem_data::data_component_impl::{DataComponentImpl, ItemNameImpl, MapIdImpl};
        use verdantgolem_data::structures::{StructureKeys, StructureSet};
        use verdantgolem_world::generation::generator::structure_finder::find_nearest_structure_start;
            ...
            while added < trade_set.amount && !remaining_trades.is_empty() {
                ...
                match trade.modifier {
                    ...
                    VillagerTradeModifier::RandomPotion => {
                        let Some(potion_name) = verdantgolem_data::tag::Potion::MINECRAFT_TRADEABLE
                            .0
                            .choose(&mut rng)
                        else {
                            continue;
                ...
                new_offers.push(verdantgolem_protocol::java::client::play::MerchantOffer {
    ...
    fn work_at_job_site(&self, game_time: i64, day_time: i64, day: i64) {
...

        {
            ...
            if !offers
                .iter()
                .any(verdantgolem_protocol::java::client::play::MerchantOffer::needs_restock)
            {
                return;
            }
            for offer in offers.iter_mut() {
                ...
                < 2.0f64.powi(2)
        {
            ...
            if let Some(claimed_profession) = profession_for_block(block) {
                ...
                world.send_entity_status(
                    self.get_entity(),
                    verdantgolem_data::entity::EntityStatus::VillagerHappy,
    ...
    pub fn set_unhappy(&self) {
        ...
        entity.world.load().send_entity_status(
            entity,
            verdantgolem_data::entity::EntityStatus::VillagerAngry,
            Some(ActorEventID::VillagerAngry),
        );
        entity.play_sound(verdantgolem_data::sound::Sound::EntityVillagerNo);
    }
    ...
    fn bedrock_trade_data(
        offers: &[verdantgolem_protocol::java::client::play::MerchantOffer],
        level: i32,
    ) -> NbtCompound {
        use verdantgolem_nbt::tag::NbtTag;
    ...
    fn send_trade_offers(
        ...
        use verdantgolem_protocol::{bedrock::client::CUpdateTrade, codec::var_long::VarLong};
...
impl ScreenHandlerFactory for VillagerEntity {
    fn create_screen_handler(
        &self,
        sync_id: u8,
        player_inventory: &Arc<verdantgolem_inventory::player::player_inventory::PlayerInventory>,
        ...
        handler.on_trade_updated = Some(Box::new(move |has_result| {
                ...
                .is_ok()
            {
                villager.get_entity().play_sound(if has_result {
                    verdantgolem_data::sound::Sound::EntityVillagerYes
                } else {
                    verdantgolem_data::sound::Sound::EntityVillagerNo
                });
            }
        }));
...
impl VillagerEntity {
    ...
    pub fn villager_mob_tick(&self) {
        ...
        if let Some(current_home) = self.get_home_pos() {
            ...
            let valid = if block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_BEDS) {
            ...
            if !valid {
                *self
                ...
                if is_sleeping {
                    ...
                    self.get_entity().send_meta_data(
                        &[Metadata::new(
                            verdantgolem_data::tracked_data::villager::SLEEPING_POS_ID,
                            None::<BlockPos>,
                        )],
                        None,
                    );
        ...
        if self.get_home_pos().is_none() {
            ...
            for entity in nearby_entities {
                if entity.get_entity().entity_id != self.get_entity().entity_id
                    && entity.get_entity().entity_type
                        == &verdantgolem_data::entity::EntityType::VILLAGER
            ...
            for p in BlockPos::iterate(start, end) {
                ...
                if block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_BEDS) {
                    let bed_props = BedProperties::from_state_id(state.id, block);
                ...
                if has_bed && has_worked {
                    ...
                    if nearby_villagers >= 3 {
                        ...
                        let nearby_golems = world
                            ...
                            .filter(|e| {
                                e.get_entity().entity_type
                                    == &verdantgolem_data::entity::EntityType::IRON_GOLEM
                            })
                            .count();
...
impl Mob for VillagerEntity {
            ...
            for tag in recipes {
                if let Some(recipe) = tag.extract_compound() {
                        ...
                        && buy_b.as_ref().is_none_or(|stack| !stack.is_empty())
                    {
                        ...
                        offers.push(verdantgolem_protocol::java::client::play::MerchantOffer {
    ...
    fn on_damage(
        &self,
        _damage_type: verdantgolem_data::damage::DamageType,
        source: Option<&dyn EntityBase>,
    ) {
        let Some(source) = source.filter(|source| {
            source.get_entity().entity_type == &verdantgolem_data::entity::EntityType::PLAYER
        }) else {
            return;
        };
    ...
    fn mob_interact(&self, player: &Arc<Player>, _item_stack: &mut ItemStack) -> bool {
        ...
        player.increment_stat(
            ...
            verdantgolem_data::statistic::CustomStatistic::TalkedToVillager as i32,
            1,
        );
...
mod tests {
    use verdantgolem_data::data_component_impl::{EnchantmentsImpl, StoredEnchantmentsImpl};
    use verdantgolem_data::villager::VillagerTradeModifier;
    use verdantgolem_util::version::JavaMinecraftVersion;
