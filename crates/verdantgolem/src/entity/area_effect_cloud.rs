...
struct ParticleMeta<'a> {
    particle_id: verdantgolem_protocol::codec::var_int::VarInt,
    data: &'a [u8],
}

impl verdantgolem_protocol::java::client::play::MetadataSerializer for ParticleMeta<'_> {
    fn write_metadata(
        &self,
        writer: &mut impl std::io::Write,
        _version: &verdantgolem_util::version::JavaMinecraftVersion,
    ) -> Result<(), verdantgolem_protocol::ser::WritingError> {
        use verdantgolem_protocol::ser::NetworkWriteExt;
...
impl EntityBase for AreaEffectCloudEntity {
    fn init_data_tracker(&self) {
        ...
        let meta = ParticleMeta {
            particle_id: verdantgolem_protocol::codec::var_int::VarInt(
                verdantgolem_data::particle::Particle::EntityEffect as i32,
            ),
            data: &data_bytes,
        };
        ...
        self.entity.send_meta_data(
            &[verdantgolem_protocol::java::client::play::Metadata::new(
                verdantgolem_data::tracked_data::area_effect_cloud::PARTICLE,
                &meta,
            )],
            None,
        );
        ...
        self.entity.send_meta_data(
            &[verdantgolem_protocol::java::client::play::Metadata::new(
                verdantgolem_data::tracked_data::area_effect_cloud::RADIUS,
                radius,
            )],
            None,
        );
        ...
        self.entity.send_meta_data(
            &[verdantgolem_protocol::java::client::play::Metadata::new(
                verdantgolem_data::tracked_data::area_effect_cloud::WAITING,
                is_waiting,
            )],
            None,
        );
    ...
    fn tick(&self, _caller: &dyn EntityBase, _server: &Server) {
        ...
        for cand in candidates {
            ...
            if *cand_clone.get_entity().entity_type
                == verdantgolem_data::entity::EntityType::AREA_EFFECT_CLOUD
            {
                continue;
            }
            ...
            if let Some(living_ref) = cand_clone.get_living_entity() {
                for (eff, _, _, _, _, _) in &effs_clone {
                    ...
                    let is_instant = eff.id
                        ...
                        || eff.id == verdantgolem_data::effect::StatusEffect::INSTANT_HEALTH.id;
                    if is_instant {
                        will_apply = true;
                        break;
                    }
