...
use verdantgolem_util::math::{boundingbox::BoundingBox, vector2::Vector2};
use verdantgolem_util::math::{position::BlockPos, vector2::to_chunk_pos};
...
fn is_water_potion(stack: &ItemStack) -> bool {
    stack
        ...
        == Some(verdantgolem_data::potion::Potion::WATER.id as i32)
}
...
impl EntityBase for SplashPotionEntity {
    fn init_data_tracker(&self) {
        ...
        entity.send_meta_data(
            &[verdantgolem_protocol::java::client::play::Metadata::new(
                verdantgolem_data::tracked_data::splash_potion::ITEM_STACK,
                &verdantgolem_protocol::codec::item_stack_seralizer::ItemStackSerializer::from(
                    stack.clone(),
                ),
            )],
            None,
    ...
    fn on_hit(&self, hit: crate::entity::projectile::ProjectileHit) {
        ...
        world.send_entity_status(
            ...
            Some(verdantgolem_protocol::bedrock::server::actor_event::ActorEventID::Death),
        );
        ...
        let has_instant = effects.iter().any(|(e, _, _, _, _, _)| {
            e.id == verdantgolem_data::effect::StatusEffect::INSTANT_DAMAGE.id
                || e.id == verdantgolem_data::effect::StatusEffect::INSTANT_HEALTH.id
        });
