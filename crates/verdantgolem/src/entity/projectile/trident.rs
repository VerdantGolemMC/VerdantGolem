...
use verdantgolem_protocol::java::client::play::{CEntityVelocity, CSoundEffect};
...
impl TridentEntity {
    ...
    fn should_skip_collision(&self, self_ent: &Entity, other: &Arc<dyn EntityBase>) -> bool {
        ...
        if other_ent.entity_type == &verdantgolem_data::entity::EntityType::ARROW
            || other_ent.entity_type == &verdantgolem_data::entity::EntityType::TRIDENT
...
fn get_hit_face(hit_pos: Vector3<f64>, block_pos: BlockPos) -> verdantgolem_data::BlockDirection {
    ...
    if local.x <= eps {
        verdantgolem_data::BlockDirection::West
    } else if local.x >= 1.0 - eps {
        verdantgolem_data::BlockDirection::East
    } else if local.y <= eps {
        verdantgolem_data::BlockDirection::Down
    } else if local.y >= 1.0 - eps {
        verdantgolem_data::BlockDirection::Up
    } else if local.z <= eps {
        verdantgolem_data::BlockDirection::North
    } else {
        verdantgolem_data::BlockDirection::South
    }
}
