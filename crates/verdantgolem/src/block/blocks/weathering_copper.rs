...
use verdantgolem_data::{Block, BlockId, BlockState, BlockStateId, Mirror, Rotation};
...
pub fn with_properties_of(
    ...
    // 2. Stairs
    if from_block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_STAIRS) {
        let props = OakStairsLikeProperties::from_state_id(from_state_id, from_block);
        return props.to_state_id(to_block);
    }

    // 3. Slabs
    if from_block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_SLABS) {
        let props = ResinBrickSlabLikeProperties::from_state_id(from_state_id, from_block);
        return props.to_state_id(to_block);
    }

    // 4. Doors
    if from_block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_DOORS) {
        let props = OakDoorLikeProperties::from_state_id(from_state_id, from_block);
        return props.to_state_id(to_block);
    }

    // 5. Trapdoors
    if from_block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_TRAPDOORS) {
...
pub fn change_over_time(world: &Arc<World>, position: &BlockPos, block: &Block) {
    ...
    if block.has_tag(&verdantgolem_data::tag::Block::MINECRAFT_DOORS) {
