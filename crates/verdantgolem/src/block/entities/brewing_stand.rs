...
use verdantgolem_data::potion_brewing::{ITEM_RECIPES, POTION_RECIPES};
...
use verdantgolem_world::inventory::{Inventory, sync_read_items_from_nbt, sync_write_items_to_nbt};
...
pub struct BrewingStandBlockEntity {
    ...
    pub ingredient_item: StdMutex<Option<&'static verdantgolem_data::item::Item>>,
}
            ...
            for slot_idx in 0..3usize {
                ...
                for recipe in &ITEM_RECIPES {
                        ...
                        && recipe.ingredient().iter().any(|i| i.id == ingredient_id)
                    {
                        ...
                        let pc = slot.get_data_component::<verdantgolem_data::data_component_impl::PotionContentsImpl>().cloned();
...
impl verdantgolem_world::inventory::Inventory for BrewingStandBlockEntity {
    fn size(&self) -> usize {
        Self::INVENTORY_SIZE
    }
...
impl verdantgolem_world::inventory::Clearable for BrewingStandBlockEntity {
    fn clear(&self) {
        let mut items = self
            .items
            .write()
        ...
        for (i, slot) in items.iter().take(3).enumerate() {
            current[i] = !slot.is_empty()
                && (slot
                    .get_data_component::<verdantgolem_data::data_component_impl::PotionContentsImpl>()
                    .is_some()
                    || slot.get_item().id == verdantgolem_data::item::Item::GLASS_BOTTLE.id);
        }
    ...
    fn tick(&self, world: &Arc<crate::world::World>) {
        ...
        if let Ok(items_guard) = self.items.try_read() {
            for (i, slot) in items_guard.iter().take(3).enumerate() {
                ...
                current[i] = !slot.is_empty() && (slot.get_data_component::<verdantgolem_data::data_component_impl::PotionContentsImpl>().is_some() || slot.get_item().id == Item::GLASS_BOTTLE.id);
            }
        }
        ...
        if needs_update {
            ...
            let mut props =
                verdantgolem_data::block_properties::BrewingStandLikeProperties::from_state_id(
                    state.id, block,
                );
