...
impl EndGatewayBlockEntity {
    ...
    pub fn get_portal_position(
        ...
        if exit_portal_opt.is_none()
            && current_level.dimension == verdantgolem_data::dimension::Dimension::THE_END
        {
            let exit_portal_pos =
        ...
        level
            ...
            .read_chunk_sync(&chunk_pos, |chunk| {
                ...
                sections.iter().all(|s| match s {
                    verdantgolem_world::chunk::palette::BlockPalette::Homogeneous(id) => {
                        verdantgolem_data::block_properties::is_air(*id)
                    }
                    verdantgolem_world::chunk::palette::BlockPalette::Heterogeneous(_) => false,
                })
            })
            .unwrap_or(true)
    }
