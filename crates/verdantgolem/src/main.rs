...
async fn main() {
    ...
    verdantgolem::init_logger(&config.advanced);

    info!(
        "{}",
        TextComponent::text(format!(
            "Starting {} {} Java Minecraft (Protocol {}) | {} Bedrock (Protocol {})",
            TextComponent::text("VerdantGolem")
    ...
    if cfg!(debug_assertions) {
        warn!(
            "VerdantGolem is running an unoptimized debug build. Do not use this build for performance testing; run `cargo run --release` or use a release binary."
        );
    }
...
fn print_support_links_and_warning() {
    warn!(
        "{}",
        TextComponent::text("VerdantGolem is currently under heavy development!")
            .color_named(NamedColor::DarkRed)
            .to_pretty_console(),
    );
    info!(
        "Report issues on {}",
        TextComponent::text("https://github.com/VerdantGolemMC/VerdantGolem/issues")
