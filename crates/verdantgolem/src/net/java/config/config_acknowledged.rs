...
pub(crate) fn build_dimension_nbt(dim: &verdantgolem_data::dimension::Dimension) -> Vec<u8> {
    ...
    let mut monster_spawn = verdantgolem_nbt::compound::NbtCompound::new();
    monster_spawn.put_string("type", "minecraft:uniform".to_string());
    let mut value = verdantgolem_nbt::compound::NbtCompound::new();
    ...
    verdantgolem_nbt::Nbt::from(compound).write().to_vec()
}
