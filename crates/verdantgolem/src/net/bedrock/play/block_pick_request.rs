...
impl BedrockClient {
    ...
    pub fn handle_block_pick_request(&self, player: &Arc<Player>, packet: &SBlockPickRequest) {
        ...
        let Some(item) = verdantgolem_data::item::Item::from_id(block.item_id) else {
            return;
        };
        ...
        if slot_with_stack != -1 {
            if verdantgolem_inventory::player::player_inventory::PlayerInventory::is_valid_hotbar_index(
