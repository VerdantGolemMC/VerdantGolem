...
pub fn java_packet(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        impl #impl_generics crate::packet::MultiVersionJavaPacket for #name #ty_generics #where_clause {
            #[must_use]
            #[inline]
            fn to_id(version: verdantgolem_util::version::JavaMinecraftVersion) -> i32 {
...
pub fn pumpkin_block(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let generated = quote! {
        impl #impl_generics crate::block::BlockMetadata for #name #ty_generics #where_clause {
            fn ids() -> Box<[verdantgolem_data::BlockId]> {
                [verdantgolem_data::BlockId::#const_ident].into()
            }
        }
    };
...
pub fn pumpkin_block_from_tag(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics crate::block::BlockMetadata for #name #ty_generics #where_clause {
            fn ids() -> Box<[verdantgolem_data::BlockId]> {
                Box::new([ #(verdantgolem_data::BlockId::#const_values),* ])
            }
        }
    };
...
//     let code = quote! {
//         #item
//         impl #impl_generics verdantgolem_world::block::properties::BlockPropertyMetadata for #name #ty_generics {
...
fn eval_translation_key_expr(expr: &syn::Expr) -> Option<(&'static str, proc_macro2::Span)> {
    match expr {
        syn::Expr::Path(expr_path) => {
            ...
            let key = if is_java {
                verdantgolem_data::translation::java::get(const_ident)
                    .and_then(verdantgolem_data::translation::java::get_value)
            } else {
                verdantgolem_data::translation::bedrock::get(const_ident)
                    .and_then(verdantgolem_data::translation::bedrock::get_value)
            };
...
pub fn translate_cross(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        {
            #[allow(deprecated)]
            verdantgolem_util::text::TextComponent::translate_cross(#java_expr, #bedrock_expr, vec![#(#args),*])
        }
    };
...
#[deprecated(
    since = "0.1.0",
    note = "Use `verdantgolem_macros::translate_cross!` macro instead for cross-platform (Java & Bedrock) translation support."
)]
#[proc_macro]
pub fn translate_java(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        {
            #[allow(deprecated)]
            verdantgolem_util::text::TextComponent::translate(#java_expr, vec![#(#args),*])
        }
    };
