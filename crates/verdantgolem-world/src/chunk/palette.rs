...
pub fn bedrock_water_state(id: BlockStateId) -> BlockStateId {
    ...
    if state.is_waterlogged() || (state.is_liquid() && Fluid::from_state_id(id).is_none()) {
        verdantgolem_data::Block::WATER.default_state.id
    } else {
        verdantgolem_data::Block::AIR.default_state.id
    }
}
        ...
        match self {
            Self::Homogeneous(registry_id) => BeNetworkSerialization {
                ...
                palette: NetworkPalette::Single(
                    verdantgolem_data::biome::Biome::from_id(*registry_id)
                ...
                BeNetworkSerialization {
                    ...
                    palette: NetworkPalette::Indirect(
                        data.palette
                            .iter()
                            .map(|&id| {
                                verdantgolem_data::biome::Biome::from_id(id)
                                    .map_or(1, |b| b.be_network_id)
                            })
                            .collect::<Vec<_>>()
                            .into_boxed_slice(),
...
impl BlockPalette {
    ...
    pub fn convert_be_water_network(&self) -> Option<BeNetworkSerialization<u16>> {
        let air = verdantgolem_data::Block::AIR.default_state.id;
        let water = verdantgolem_data::Block::WATER.default_state.id;
        match self {
            Self::Homogeneous(id) => {
