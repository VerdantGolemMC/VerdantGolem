    ...
    fn dimensions_taller_than_their_noise_settings_generate_all_sections() {
        for (dimension, terrain_state) in [
            (
                ...
                verdantgolem_data::Block::NETHERRACK.default_state.id,
            ),
            (
                Dimension::THE_END,
                verdantgolem_data::Block::END_STONE.default_state.id,
            ),
        ] {
            let seed = Seed(42);
            ...
            for z in -192..-176 {
                for y in -64..320 {
                    let block = chunk
                        ...
                        .to_block_id();
                    if [
                        verdantgolem_data::Block::DEEPSLATE_BRICKS.id,
                        verdantgolem_data::Block::POLISHED_DEEPSLATE.id,
                        verdantgolem_data::Block::REINFORCED_DEEPSLATE.id,
                        verdantgolem_data::Block::SCULK.id,
                    ]
                    .contains(&block)
                    {
                        city_blocks += 1;
                    }
                    if block == verdantgolem_data::Block::JIGSAW.id {
                        jigsaw_blocks += 1;
                    }
                }
            }
            ...
            for z in -1328..-1312 {
                for y in -64..320 {
                    let block = chunk
                        ...
                        .to_block_id();
                    if [
                        verdantgolem_data::Block::DARK_OAK_LOG.id,
                        verdantgolem_data::Block::DARK_OAK_PLANKS.id,
                        verdantgolem_data::Block::DARK_OAK_FENCE.id,
                    ]
                    .contains(&block)
                    {
                        outpost_blocks += 1;
