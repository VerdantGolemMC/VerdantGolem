...
impl PoiRegion {
    ...
    fn compress_chunk_data(chunk_data: &PoiChunkData) -> std::io::Result<Vec<u8>> {
        let mut root = verdantgolem_nbt::compound::NbtCompound::new();
        root.put_int("DataVersion", chunk_data.data_version);

        let mut sections_comp = verdantgolem_nbt::compound::NbtCompound::new();
        for (sec_key, sec_data) in &chunk_data.sections {
            let mut sec_comp = verdantgolem_nbt::compound::NbtCompound::new();
            sec_comp.put_byte("Valid", sec_data.valid);
            let mut rec_list = Vec::new();
            for rec in &sec_data.records {
                let mut rec_comp = verdantgolem_nbt::compound::NbtCompound::new();
                ...
                rec_list.push(verdantgolem_nbt::tag::NbtTag::Compound(rec_comp));
            }
        ...
        let uncompressed = verdantgolem_nbt::Nbt::from(root).write();
    ...
    fn decompress_chunk_data(compressed: &[u8]) -> std::io::Result<PoiChunkData> {
        ...
        let nbt = verdantgolem_nbt::Nbt::read(&mut reader)
