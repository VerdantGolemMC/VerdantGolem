...
use verdantgolem_data::fluid::{Fluid, FluidState};
use verdantgolem_data::structures::{
    Structure, StructureKeys, StructurePlacementType, StructureSet, WeightedEntry,
};
use verdantgolem_data::tag::RegistryKey;
use verdantgolem_data::{Block, BlockState, block_properties::blocks_movement, chunk::Biome};
use verdantgolem_data::{BlockId, BlockStateId, tag};
use verdantgolem_util::random::xoroshiro128::XoroshiroSplitter;
use verdantgolem_util::random::{RandomImpl, get_carver_seed};
...
use verdantgolem_data::tag::get_tag_ids;
        ...
        for (set_index, set) in StructureSet::ALL.iter().enumerate() {
            ...
            match &set.placement.placement_type {
                StructurePlacementType::RandomSpread(spread) => {
                    let region_x = verdantgolem_util::math::floor_div(self.x, spread.spacing);
                    let region_z = verdantgolem_util::math::floor_div(self.z, spread.spacing);
