...
pub static CONFIGURED_FEATURES: LazyLock<
    HashMap<verdantgolem_data::configured_feature::ConfiguredFeature, ConfiguredFeature>,
> = LazyLock::new(build_configured_features);

pub static BONE_MEAL_FEATURES: LazyLock<
    HashSet<verdantgolem_data::configured_feature::ConfiguredFeature>,
> = LazyLock::new(|| {
    verdantgolem_data::tag::get_tag_values(
        verdantgolem_data::tag::RegistryKey::WorldgenConfiguredFeature,
        "minecraft:can_spawn_from_bone_meal",
    )
    .into_iter()
    .flatten()
    .filter_map(|name| verdantgolem_data::configured_feature::ConfiguredFeature::from_name(name))
    .collect()
});
...
include!("../../../../verdantgolem-data/src/generated/configured_features_generated.rs");
...
mod tests {
    ...
    fn bonemeal_features_use_tag_output() {
        let tag_values = verdantgolem_data::tag::get_tag_values(
            verdantgolem_data::tag::RegistryKey::WorldgenConfiguredFeature,
            "minecraft:can_spawn_from_bone_meal",
        );
