...
pub fn build() -> TokenStream {
    ...
    quote! {
        ...
        use verdantgolem_util::math::int_provider::{UniformIntProvider, IntProvider, NormalIntProvider};
        use verdantgolem_util::loot_table::*;
        use verdantgolem_util::math::experience::Experience;
...
fn get_be_data_from_nbt<R: Read + Seek>(
    ...
    loop {
        ...
        let nbt = verdantgolem_nbt::Nbt::read(nbt_reader).unwrap();
        ...
        let properties = nbt
            ...
            .map(|(key, val)| {
                let unpacked = match val {
                    verdantgolem_nbt::tag::NbtTag::Byte(v) => {
                        if v == 1 {
                            "true".into()
                        } else {
                            "false".into()
                    ...
                    verdantgolem_nbt::tag::NbtTag::Int(v) => v.to_string(),
                    verdantgolem_nbt::tag::NbtTag::String(v) => v.into(),
                    _ => {
...
fn parse_geyser_entry(
    entry: &verdantgolem_nbt::tag::NbtTag,
    java_block_name: &str,
) -> (String, BTreeMap<String, String>) {
    let compound = match entry {
        verdantgolem_nbt::tag::NbtTag::Compound(c) => c,
    ...
    if let Some(state_comp) = compound.get_compound("state") {
        for (key, val) in state_comp.clone() {
            let unpacked = match val {
                ...
                verdantgolem_nbt::tag::NbtTag::Int(v) => v.to_string(),
                verdantgolem_nbt::tag::NbtTag::String(v) => v.to_string(),
                _ => {
                    panic!(
