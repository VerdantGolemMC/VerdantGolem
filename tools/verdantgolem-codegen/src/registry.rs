            ...
            .map(|(reg_name, entries)| {
                let entry_tokens: Vec<TokenStream> = entries
                    ...
                    .map(|(entry_name, entry_data)| {
                        fn json_to_nbt_tag(v: &Value) -> verdantgolem_nbt::tag::NbtTag {
                            match v {
                                Value::Null => verdantgolem_nbt::tag::NbtTag::End,
                                Value::Bool(b) => {
                                    verdantgolem_nbt::tag::NbtTag::Byte(if *b { 1 } else { 0 })
                                }
                                Value::Number(num) => {
                                    if let Some(i) = num.as_i64() {
                                        if i >= i32::MIN as i64 && i <= i32::MAX as i64 {
                                            verdantgolem_nbt::tag::NbtTag::Int(i as i32)
                                        } else {
                                            verdantgolem_nbt::tag::NbtTag::Long(i)
                                        }
                                    } else if let Some(f) = num.as_f64() {
                                        verdantgolem_nbt::tag::NbtTag::Double(f)
                                    } else {
                                        verdantgolem_nbt::tag::NbtTag::Int(0)
                                    }
                                }
                                Value::String(s) => {
                                    verdantgolem_nbt::tag::NbtTag::String(s.clone().into())
                                }
                                Value::Array(arr) => verdantgolem_nbt::tag::NbtTag::List(
