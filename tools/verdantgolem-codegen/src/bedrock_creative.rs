...
pub fn build() -> TokenStream {
    ...
    if !std::path::Path::new(nbt_path).exists() {
        let generated_path = "../../crates/verdantgolem-data/src/generated/bedrock_creative.rs";
    ...
    let mut reader = verdantgolem_nbt::deserializer::NbtReadHelperBedrock::new(
        verdantgolem_nbt::deserializer::NbtStreamReader(&mut cursor),
    );
    let nbt = verdantgolem_nbt::Nbt::read(&mut reader).expect("Failed to read creative_items.nbt");

    let mut group_tokens = Vec::new();
    if let Some(verdantgolem_nbt::tag::NbtTag::List(groups_list)) = nbt.get("groups") {
        for g_tag in groups_list {
            if let verdantgolem_nbt::tag::NbtTag::Compound(g) = g_tag {
                ...
                if let Some(verdantgolem_nbt::tag::NbtTag::Compound(icon)) = g.get("icon") {
    ...
    if let Some(verdantgolem_nbt::tag::NbtTag::List(items_list)) = nbt.get("items") {
        for item_tag in items_list {
            if let verdantgolem_nbt::tag::NbtTag::Compound(item) = item_tag {
