    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            Self::Absolute { absolute } => {
                tokens.extend(quote!(YOffset::Absolute(verdantgolem_util::y_offset::Absolute { absolute: #absolute })));
            }
            Self::AboveBottom { above_bottom } => {
                tokens.extend(quote!(YOffset::AboveBottom(verdantgolem_util::y_offset::AboveBottom { above_bottom: #above_bottom })));
            }
            Self::BelowTop { below_top } => {
                tokens.extend(quote!(YOffset::BelowTop(verdantgolem_util::y_offset::BelowTop { below_top: #below_top })));
            }
        }
    }
}
        ...
        match self {
            ...
            Self::StoneDepth {
                ...
                let surface_type_token = match surface_type.as_str() {
                    "ceiling" => quote!(
                        verdantgolem_util::math::vertical_surface_type::VerticalSurfaceType::Ceiling
                    ),
                    "floor" => quote!(
                        verdantgolem_util::math::vertical_surface_type::VerticalSurfaceType::Floor
                    ),
...
pub fn build() -> TokenStream {
    ...
    quote!(
        ...
        use verdantgolem_util::random::RandomDeriver;
        use verdantgolem_util::y_offset::YOffset;
        use crate::biome::{Biome, Parameter, ParameterPoint};
        use verdantgolem_util::y_offset::Absolute;
        ...
        pub struct StoneDepthMaterialCondition {
            ...
            pub surface_type: verdantgolem_util::math::vertical_surface_type::VerticalSurfaceType,
        }
